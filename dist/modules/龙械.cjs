...
function 取节奏(节值, 间隔 = 100) {
  if (!节值) return stagger.stagger(间隔, { from: 'first' })
  if (typeof 节值 === 'number') return stagger.stagger(节值, { from: 'first' })
  if (typeof 节值 === 'string' && 节[节值]) return stagger.stagger(间隔, 节[节值])
  if (typeof 节值 === 'object') return stagger.stagger(间隔, 节值)
  return stagger.stagger(间隔, { from: 'first' })
}
...
function 叠(目标, 选项 = {}) {
  ...
  if (参.delay === undefined && !选项.节奏) {
    参.delay = stagger.stagger(150, { from: 'first' });
  }
  return animation.animate(目标, 参)
}
 ...
 */
function 谱(选项 = {}) {
  return timeline.createTimeline(选项)
}
 ...
 */
function 械(目标, 选项 = {}) {
  return animation.animate(目标, 选项)
}
